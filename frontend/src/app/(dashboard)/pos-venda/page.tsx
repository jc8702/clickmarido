...
import {
  ...
  syncFollowUps,
  triggerCronManually,
} from '@/lib/api/modules/follow-ups';
...
export default function PosVendaPage() {
  ...
  const renderStatusStep = (
    sent: boolean,
    daysRequired: number,
    completionDateStr: string | undefined,
  ) => {
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Régua automatizada de relacionamento via WhatsApp.
          </p>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={handleTrigger}
            className="bg-amber-500 text-white px-4 py-2 rounded-md hover:bg-amber-600 font-medium text-sm"
          >
          ...
          <button
            onClick={handleSync}
            disabled={syncing}
            className="flex items-center gap-2 bg-emerald-500 text-white px-4 py-2 rounded-md hover:bg-emerald-600 font-medium text-sm disabled:opacity-50"
          >
        ...
        <table className="w-full text-sm text-left">
          <thead className="bg-muted/50 border-b border-border/50">
            <tr>
              ...
              <th className="px-6 py-4 font-bold text-muted-foreground uppercase text-xs text-center">
                Avaliação (1d)
              </th>
              <th className="px-6 py-4 font-bold text-muted-foreground uppercase text-xs text-center">
                Pesquisa (7d)
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={6} className="px-6 py-8 text-center text-muted-foreground">
                  Nenhuma Ordem de Serviço em pós-venda.
                </td>
              </tr>
            ) : (
              followUps.map((f) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">{renderStatusStep(f.sent1Day, 1, completionDate)}</td>
                    <td className="px-6 py-4">
