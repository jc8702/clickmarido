...
function ServicosPageInner() {
  const {
    services, total, page, totalPages, search, categoryFilter, statusFilter, loading,
    setPage, setSearch, setCategoryFilter, setStatusFilter, handleDelete, fetchServices,
  } = useServices();
    ...
    <div className="p-8 lg:p-12 max-w-6xl mx-auto space-y-10 animate-in-fade">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-zinc-900 pb-8">
        ...
        <div className="flex flex-wrap w-full md:w-auto gap-3">
          <Button onClick={handleExportCsv} disabled={exportLoading} variant="outline" className="h-11 px-4 rounded-xl border-zinc-800 bg-zinc-900/50 hover:bg-zinc-850 hover:text-white text-zinc-300 font-bold flex items-center gap-2 disabled:opacity-50">
            <Download className="w-4 h-4" /> Exportar CSV
          </Button>
          <Button onClick={() => setImportModalOpen(true)} variant="outline" className="h-11 px-4 rounded-xl border-zinc-800 bg-zinc-900/50 hover:bg-zinc-850 hover:text-white text-zinc-300 font-bold flex items-center gap-2">
            <Upload className="w-4 h-4" /> Importar CSV
          </Button>
          <Button onClick={() => setFormModal({ open: true, service: null })} className="h-11 px-6 rounded-xl bg-violet-600 hover:bg-violet-500 shadow-lg shadow-violet-500/20 font-bold shrink-0 flex items-center gap-2">
      ...
      {loading ? (
        ...
        <Card className="flex flex-col items-center justify-center p-24 text-center border-dashed border-zinc-800 glass-card">
          ...
          <p className="text-zinc-500 mt-2 max-w-sm">Tente ajustar os filtros ou cadastre um novo serviço no catálogo.</p>
        </Card>
      ) : (
      ...
      <ServiceFormModal isOpen={formModal.open} onClose={() => setFormModal({ open: false, service: null })} onSuccess={fetchServices} service={formModal.service} />
      <ServiceImportModal isOpen={importModalOpen} onClose={() => setImportModalOpen(false)} onSuccess={fetchServices} />
    </div>
  );
}
