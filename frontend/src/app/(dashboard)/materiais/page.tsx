...
function MateriaisPageInner() {
  const {
    materials, loading, search, categoryFilter, page, totalPages, categories,
    setSearch, setCategoryFilter, setPage, fetchMaterials, handleDelete,
  } = useMaterials();

  const [formModal, setFormModal] = useState({ open: false, material: null as Material | null });
  const [movementModal, setMovementModal] = useState({ open: false, material: null as Material | null });
  const [historyModal, setHistoryModal] = useState({ open: false, material: null as Material | null });
  ...
  return (
    <div className="p-8 max-w-[1600px] mx-auto space-y-8 animate-in-fade">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <Button onClick={() => setFormModal({ open: true, material: null })}
      ...
      <MaterialsTable
        materials={materials} loading={loading} page={page} totalPages={totalPages}
      ...
      <MaterialFormModal isOpen={formModal.open} onClose={() => setFormModal({ open: false, material: null })}
        onSuccess={fetchMaterials} material={formModal.material} categories={[...categories]} />
      <MaterialMovementModal isOpen={movementModal.open} onClose={() => setMovementModal({ open: false, material: null })}
        onSuccess={fetchMaterials} material={movementModal.material} />
      <MaterialHistoryModal isOpen={historyModal.open} onClose={() => setHistoryModal({ open: false, material: null })}
