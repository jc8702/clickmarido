    ...
    <div className="flex h-[calc(100vh-4rem)] p-4 md:p-6 gap-6">
      ...
      <div className="w-1/3 min-w-[300px] flex flex-col glass-card border-border/50 rounded-xl shadow-sm overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            <div className="p-4 text-center text-muted-foreground text-sm">Carregando conversas...</div>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-center mb-1">
                    <h4 className="font-bold text-sm truncate">{chat.client?.name || chat.contactName || chat.contactNumber}</h4>
                    <span className="text-[10px] text-muted-foreground shrink-0">{format(new Date(chat.lastMessageAt), 'HH:mm')}</span>
                  </div>
        ...
        {selectedChat ? (
          <>
            ...
            <div className="p-4 border-b border-border/50 flex items-center gap-3 bg-muted/50">
              ...
              <div>
                <h3 className="font-bold">{selectedChat.client?.name || selectedChat.contactName || selectedChat.contactNumber}</h3>
              ...
              {messages.map(msg => (
                <div key={msg.id} className={`flex ${msg.fromMe ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[70%] rounded-xl px-4 py-2 ${msg.fromMe ? 'bg-emerald-500 text-white rounded-tr-sm' : 'bg-background border border-border/50 shadow-sm rounded-tl-sm'}`}>
                    ...
                    <div className={`text-[10px] mt-1 text-right flex items-center justify-end gap-1 ${msg.fromMe ? 'text-emerald-100' : 'text-muted-foreground'}`}>
