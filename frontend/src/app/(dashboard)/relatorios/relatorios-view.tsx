...
const CommercialChart = dynamic(
  () => import('@/components/reports/charts').then((mod) => mod.CommercialChart),
  { ssr: false, loading: () => <Skeleton className="h-[300px] w-full rounded-xl" /> },
);
const OperationalChart = dynamic(
  () => import('@/components/reports/charts').then((mod) => mod.OperationalChart),
  { ssr: false, loading: () => <Skeleton className="h-[300px] w-full rounded-xl" /> },
);
const FinancialChart = dynamic(
  () => import('@/components/reports/charts').then((mod) => mod.FinancialChart),
  { ssr: false, loading: () => <Skeleton className="h-[350px] w-full rounded-xl" /> },
);

export default function RelatoriosPage() {
  const [activeTab, setActiveTab] = useState<'commercial' | 'operational' | 'financial'>(
    'commercial',
  );
  ...
  if (loading) {
    return (
      <div className="p-8 text-muted-foreground animate-pulse">
        Carregando métricas e montando gráficos...
      </div>
    );
  }
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <div className="glass-card border-border/50 rounded-2xl p-6 shadow-sm">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {commercial.approvedQuotes} de {commercial.totalQuotes} orçamentos viraram OS
              </p>
            </div>
            ...
            <div className="glass-card border-border/50 rounded-2xl p-6 shadow-sm">
              ...
              <div className="text-3xl font-extrabold text-foreground tracking-tight">
                R$ {commercial.ticketMedio.toFixed(2)}
              </div>
              <p className="text-xs text-muted-foreground mt-1">
                Baseado em {commercial.completedOrders} ordens concluídas
              </p>
            </div>
            ...
            <div className="glass-card border-border/50 rounded-2xl p-6 shadow-sm">
              ...
              <div className="text-3xl font-extrabold text-foreground tracking-tight">
                {commercial.topServices.reduce((a, b) => a + b.value, 0)}
              </div>
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="glass-card border-border/50 rounded-2xl p-6 shadow-sm">
              ...
              <div className="text-3xl font-extrabold text-foreground tracking-tight">
                {operational.avgTimeDays} dias
              </div>
              <p className="text-xs text-muted-foreground mt-1">
                Média entre a criação e conclusão da OS
              </p>
            </div>
          </div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="glass-card border-border/50 rounded-2xl p-6 shadow-sm">
              <h3 className="font-bold text-foreground mb-6">
                Produtividade por Técnico (OS Concluídas)
              </h3>
              <div className="h-[300px] w-full">
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <div className="glass-card border-border/50 rounded-2xl p-6 shadow-sm">
              ...
              <div
                className={`text-3xl font-extrabold tracking-tight ${financial.netProfit >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}`}
              >
