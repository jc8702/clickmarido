...
const DashboardLineChart = dynamic(() => import('@/components/dashboard/charts').then(mod => mod.DashboardLineChart), {
  ssr: false,
  loading: () => <Skeleton className="h-[350px] w-full rounded-xl" />
});

const DashboardBarChart = dynamic(() => import('@/components/dashboard/charts').then(mod => mod.DashboardBarChart), {
  ssr: false,
  loading: () => <Skeleton className="h-[350px] w-full rounded-xl" />
});
