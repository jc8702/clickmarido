...
export class AppointmentsService {
  ...
  async create(createDto: CreateAppointmentDto, companyId: string) {
    ...
    if (serviceOrderId)
      await this.conflictDetector.ensureServiceOrderExists(
        serviceOrderId,
        companyId,
      );
    if (technicianId) {
      await this.conflictDetector.ensureTechnicianAndCheckConflicts(
        companyId,
        technicianId,
        start,
        end,
  ...
  async update(id: string, updateDto: UpdateAppointmentDto, companyId: string) {
    ...
    const updated = await this.repo.update(id, {
      ...
      endTime: end,
      client:
        clientId !== undefined ? { connect: { id: clientId } } : undefined,
      technician:
