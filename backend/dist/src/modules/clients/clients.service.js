...
let ClientsService = ClientsService_1 = class ClientsService {
    ...
    async create(createClientDto, companyId, userId) {
        ...
        const createdClient = await this.repo.createWithHistory({ ...createClientDto, companyId, lat, lng }, { type: 'SYSTEM', description: `Cliente cadastrado por ${userName}`, createdById: userId || null, clientId: '' });
    ...
    async update(id, updateClientDto, companyId, userId) {
        ...
        const updatedClient = await this.repo.updateWithHistory(id, dataToUpdate, { type: 'SYSTEM', description: `Cadastro atualizado por ${userName}`, createdById: userId || null, clientId: '' });
        return { success: true, data: updatedClient };
    }
    async remove(id, companyId, userId) {
        await this.validator.ensureClientExists(id, companyId);
        const userName = await this.getUserName(userId);
        await this.repo.softDeleteWithHistory(id, { type: 'SYSTEM', description: `Cliente arquivado (soft-delete) por ${userName}`, createdById: userId || null, clientId: '' });
