...
export function Sidebar({ className }: SidebarProps) {
  ...
  return (
    <aside
      className={cn(
        'flex flex-col w-64 bg-zinc-950 border-r border-zinc-900 h-screen sticky top-0',
        className
      )}
    >
      {/* Logo */}
      ...
      <nav className="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
        <div className="px-3 mb-2 text-[10px] font-semibold tracking-wider text-zinc-500 uppercase font-mono">
          Menu Principal
        </div>
        {navigation.map((item) => {
          const isActive =
      ...
      <div className="p-4 border-t border-zinc-900 bg-zinc-950/50 space-y-3">
        <div className="flex items-center justify-between p-2 rounded-md bg-zinc-900/60 border border-zinc-800 text-[10px]">
        ...
        <div className="flex items-center gap-3 pt-2 border-t border-zinc-900/50">
          ...
          <div className="flex flex-col overflow-hidden">
            ...
            <span className="text-[10px] text-zinc-500 truncate">admin@clickmarido.com</span>
          </div>
        </div>
      </div>
    </aside>
